...
test('sends reasoning_effort when the option and model both support it (glm-5.2 + max)', async () => {
  mockResponse = { ok: true, status: 200, body: { choices: [{ message: { content: 'x' } }] } }
  const client = new ZaiGlmClient({ model: 'glm-5.2' })

  await client.chat([{ role: 'user', content: 'hi' }], { reasoningEffort: 'max' })

  const body = JSON.parse(calls[0].init.body as string)
  assert.equal(body.reasoning_effort, 'max')
})

// GLM-5.2 only documents high/max — 'low' is a GLM-5.3-only value. Sending
// an unsupported value risks being silently ignored or rejected by z.ai, so
// ZaiGlmClient omits the field entirely rather than forwarding a value the
// resolved model doesn't document support for.
test('omits reasoning_effort when the requested value is not supported by the resolved model (glm-5.2 + low)', async () => {
  mockResponse = { ok: true, status: 200, body: { choices: [{ message: { content: 'x' } }] } }
  const client = new ZaiGlmClient({ model: 'glm-5.2' })

  await client.chat([{ role: 'user', content: 'hi' }], { reasoningEffort: 'low' })

  const body = JSON.parse(calls[0].init.body as string)
  assert.equal('reasoning_effort' in body, false)
})

test('omits reasoning_effort entirely for a model that does not document the parameter (glm-4.7)', async () => {
  mockResponse = { ok: true, status: 200, body: { choices: [{ message: { content: 'x' } }] } }
  const client = new ZaiGlmClient({ model: 'glm-4.7' })
...
test('sends reasoning_effort:"low" for glm-5.3, the only model that documents it', async () => {
  mockResponse = { ok: true, status: 200, body: { choices: [{ message: { content: 'x' } }] } }
  const client = new ZaiGlmClient({ model: 'glm-5.3' })
...
// The behavior change this whole capability table exists to protect
// against: GLM-5.3 cannot disable thinking at all (z.ai's own docs — always
// reasoning-on), so a request for thinking:disabled against glm-5.3 must
// NOT be forwarded — sending it would be a silent no-op that leaves a user
// believing they successfully disabled reasoning when they didn't.
test('does not send thinking:disabled for glm-5.3, which cannot disable thinking', async () => {
  mockResponse = { ok: true, status: 200, body: { choices: [{ message: { content: 'x' } }] } }
  const client = new ZaiGlmClient({ model: 'glm-5.3' })
...
test('does send thinking:enabled for glm-5.3 (a no-op confirmation, not a disable request)', async () => {
  mockResponse = { ok: true, status: 200, body: { choices: [{ message: { content: 'x' } }] } }
  const client = new ZaiGlmClient({ model: 'glm-5.3' })
